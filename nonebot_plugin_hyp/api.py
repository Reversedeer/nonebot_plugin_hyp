...
class Api:
    def __init__(self) -> None:
        ...
        self.antisniper_winstreak = (
            "https://api.antisniper.net/winstreak?key={}&name={}"
        )
    ...
    async def player_data(self, uid: str) -> dict:
        """初始化获取玩家数据"""
        mcdata: Dict[str, str] = await self.get_mc_data(uid)
        (
            player_data,
            player_status,
    ...
    async def get_player_data(self, mcdata) -> Dict[str, str]:
        """获取玩家hypixel游戏数据"""
        for _ in range(3):
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    if res.status_code == 429:
                        ...
                        return {"e": "超出API请求次数限制"}
            except Exception:
                logger.error("获取玩家hypixel游戏数据时请求超时：可能是网络状况不佳")
                return {"e": "获取玩家hypixel游戏数据发生错误：可能是网络状况不佳"}
        return {"e": "获取玩家数据时发生未知错误，请稍后重试！"}

    async def get_player_status(self, mcdata) -> Dict[str, str]:
        """获取玩家hypixel状态信息"""
        for _ in range(3):
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    res: httpx.Response = await client.get(url, timeout=10)
                    if res.status_code == 200:
                        return res.json().get("session").get("online")
    ...
    async def get_mc_data(self, uid) -> dict:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    logger.error("获取玩家minecraft信息请求超时，请稍后再试")
                    return {"e": "请求超时，请稍后重试！"}
        return {"e": "获取玩家数据时发生未知错误，请稍后重试！"}

    async def get_hypixel_data(self, data_a) -> dict:
        fields = {
            "karma": data_a.get("karma"),
            "rank": await self.get_player_rack(data_a.get("newPackageRank")),
            "first_login": await self.get_login_time(data_a.get("firstLogin")),
            "last_login": await self.get_login_time(data_a.get("lastLogin")),
            "level": await self.get_player_level(data_a.get("networkExp")),
            "achievementPoints": data_a.get("achievementPoints"),  # 成就
            "quest_master": data_a.get("achievements").get(
                "general_quest_master"
            ),  # 任务
            "challenger": data_a.get("achievements").get("general_challenger"),  # 挑战
            "general_wins": data_a.get("achievements").get(
                "general_wins"
            ),  # 小游戏胜场
            "general_coins": data_a.get("achievements").get(
                "general_coins"
            ),  # 小游戏硬币
    ...
    async def get_players_skywars(self, data_a) -> dict:
    ...
    async def get_hypixel_bedwars_level(
        ...
        if exp < sum(LEVEL_EXP_REQUIREMENTS):
            ...
            if exp < MAX_LEVEL_EXP:
                add_level = exp // POST_3_LEVEL_EXP
                remaining_exp = exp % POST_3_LEVEL_EXP
                level = 4 + add_level
                level_str = f"{level}✫"
                next_level_exp = f"{remaining_exp}/{POST_3_LEVEL_EXP}"
            else:
                # 超过 MAX_LEVEL_EXP 的特殊循环处理
                loop_count = exp // MAX_LEVEL_EXP  # 计算循环次数
                surplus_exp = exp % MAX_LEVEL_EXP  # 当前循环内的剩余经验
                base_level = loop_count * 100  # 每轮加 100 星
                loop_level, remaining_exp = calculate_progress(
                    surplus_exp,
                    LEVEL_EXP_REQUIREMENTS + [POST_3_LEVEL_EXP] * 100,
                )
                level_str = f"{base_level + loop_level}✫"
    ...
    async def get_players_bedwars(self, data_a) -> dict:
        stats_data = data_a.get("stats")
        if not stats_data:
            logger.error("Stats 数据不存在")
            return {"error": "Stats 数据不存在"}
        ...
        data_b = await api.get_hypixel_bedwars_level(
            int(bedwars_data.get("Experience", 0))
        )
        ...
        # 计算各类比值
        fields["BBLR"] = round(
            fields["break_bed"] / fields["lost_bed"],
            3,
        )
        fields["W_L"] = round(
            fields["bw_win"] / fields["bw_losses"],
            3,
        )
        fields["K_D"] = round(
            fields["bw_kill"] / fields["bw_death"],
            3,
        )
        fields["FKDR"] = round(
            fields["bw_final_kill"] / fields["bw_final_death"],
            3,
        )
